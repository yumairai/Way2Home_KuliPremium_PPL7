                ...
                <div class="sidebar-menu">
                    @forelse ($proyeks as $item)
                        <a class="sidebar-menu-item {{ request()->is('proyek/' . $item->id) ? 'active' : '' }}"
                            ...
                            <span
                                class="material-symbols-outlined {{ request()->is('proyek/' . $item->id) ? 'filled' : '' }}">
                                home_work
                            </span>
                            {{ $item->detailBangun?->desainRumah?->tipe_rumah ?? $item->jenis_proyek }}
                        </a>
                    @empty
                        <div class="sidebar-menu-empty">Tidak ada proyek</div>
                    @endforelse
                </div>
            </div>
        </aside>
...
@push('scripts')
    ...
    <script src="{{ asset('js/customer/payment_proyek.js') }}"></script>
@endpush
