...
@section('content')
    ...
    <div class="body">
        ...
        <div class="info-card">
            ...
            <div class="info-row">
                <div>
                    ...
                    <div class="value" style="font-size:13px; font-weight:400; color:#4a5568;">
                        {{ Str::limit($deskripsi, 120) }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="info-card" style="background:#f0f7ff; border-color:#bee3f8;">
            ...
            <div class="info-row">
                ...
                @if ($materialTotal > 0)
                    <div style="text-align:right">
                        ...
                        <div class="value" style="color:#2B3361;">Rp {{ number_format($materialTotal, 0, ',', '.') }}
                        </div>
                    </div>
                @endif
            </div>
            @if ($materialTotal > 0)
                <div class="info-row"
                    style="background:#dbeafe; border-radius:8px; padding:10px; border:none; margin-top:4px;">
                    <div>
                        <div class="label">Total Estimasi</div>
                        <div class="value" style="color:#1a365d; font-size:17px;">Rp
                            {{ number_format($totalBiaya, 0, ',', '.') }}</div>
                    </div>
                </div>
            @endif
        </div>
        ...
        @if ($penawaran->materialRenovasi && $penawaran->materialRenovasi->count() > 0)
            <div class="info-card" style="margin-top:0;">
                ...
                @foreach ($penawaran->materialRenovasi as $item)
                    <div
                        style="display:flex; justify-content:space-between; align-items:center; padding:8px 0; border-bottom:1px solid #edf2f7; font-size:13px;">
                        <span style="color:#2d3748;">{{ $item->material?->nama_material ?? '-' }}</span>
                        <span style="color:#718096;">{{ $item->jumlah }} {{ $item->satuan }}</span>
                        <span style="color:#2B3361; font-weight:600;">Rp
                            {{ number_format(($item->material?->harga ?? 0) * $item->jumlah, 0, ',', '.') }}</span>
                    </div>
                @endforeach
            </div>
        @endif
        ...
        <div class="cta-wrap">
            <a href="{{ route('customer.renovation') }}" class="cta-btn">
                Tinjau Penawaran
            </a>
        </div>
    </div>
