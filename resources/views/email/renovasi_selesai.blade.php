...
@section('content')
    @php
        ...
        $offer =
            $requestRenovasi->penawaran?->where('status_penawaran', 'diterima')->first() ??
    ...
    <div class="body">
        ...
        <div class="info-card" style="background:#f0fff4; border-color:#9ae6b4;">
            ...
            @foreach (['Survei & Analisis Kebutuhan', 'Pengadaan Material', 'Pengerjaan Renovasi', 'Pengecekan Kualitas', 'Pembersihan Area Kerja'] as $item)
                <div
                    style="display:flex; align-items:center; gap:10px; padding:7px 0; font-size:13px; color:#276749; border-bottom:1px solid #c6f6d5;">
                    <span>✅</span> {{ $item }}
                </div>
            @endforeach
        </div>
